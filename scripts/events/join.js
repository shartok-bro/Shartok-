...
.then(response => {
  ...
  if (event.logMessageData.addedParticipants.some(i => i.userFbId == api.getCurrentUserID())) {
    ...
    return api.sendMessage("মুবিন বট চলে আসছে 😼 সবডি মুড়ি খা 😾🖤!", event.threadID, () => api.sendMessage({ body: `${global.config.BOTNAME} CONNECTED«\n\nAssalamualaykum☘️
<------------------------------>  
...
DEVELOPER :Arafat Al Mubin

🟣Facebook Account Link: 

https://www.facebook.com/profile.php?id=61554376447644

🔵WHATSAPP NUMBER: wa.me/+8801914087519

🟢SUPPORT EMAIL: gamingmubinbm@gmail.com`, attachment: fs.createReadStream(gifPath)}, threadID));
  }})
.catch(error => {
    console.error(error);
});
